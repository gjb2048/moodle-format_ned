...
var d=a(this).find(":selected").val();switch(d){case"0":p.text(c.sectionnamenavblockvaluedata);break;case"1":p.text(k.val());break;case"2":p.text(l.val());break;case"3":p.text(m.val())}q=d}),k.on("keyup",function(b){"1"==q&&// Left column.
p.text(a(this).val())}),l.on("keyup",function(b){"2"==q&&// Middle column.
p.text(a(this).val())}),m.on("keyup",function(b){"3"==q&&// Right column.
p.text(a(this).val())}),this.on("change",function(b){r(a(this))})}}(a),{init:function(c){a(document).ready(function(a){a("select#id_sectionheaderformat").sectionHeaderFormat(c),window.JSON&&window.JSON.stringify&&b.debug("NED Format Edit Section Form AMD data: "+JSON.stringify(c))}),b.debug("NED Format Edit Section Form AMD init.")}}});
...
class format_ned extends format_base {
    ...
    public function get_settings() {
        ...
        if (empty($this->settings) == true) {
                ...
                while ($section <= $numsections) {
                    if ($headerformats && isset($headerformats[$section])) {
                        ...
                        $this->sectionheaderformatheaders[$section]
                            = json_decode($headerformatdefault, true);
                    }
                    $section++;
                }
            }
            ...
            $this->settings['compressedmodeview'] = get_config('format_ned', 'compressedmodeview');
    ...
    public function get_setting($name, $section = null) {
        ...
        } else if ($name == 'compressedmodeview') {
            return $this->settings['compressedmodeview'];
    ...
    protected function update_format_options($data, $sectionid = null) {
        global $DB;
        if ((!empty($data['headerformat'])) && ($sectionid) && ($this->settings['sectionformat'] == 3)) {

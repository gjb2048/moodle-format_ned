...
$countsql = "SELECT COUNT(1) FROM {format_ned_colour} tc WHERE 0 = 0 $where";
...
foreach ($tablerows as $tablerow) {
    ...
    foreach ($columns as $column) {
        ...
        switch ($column) {
            ...
            case 'predefined':
                if ($tablerow->$column > 0) {
                    $$varname = new html_table_cell(get_string('yes', 'format_ned'));
                } else {
...
if (has_capability('moodle/course:update', $coursecontext)) {
    ...
    $submitbutton  = html_writer::tag('button', get_string('add', 'format_ned'), array(
    ...
    $formurlclose = new moodle_url('/course/format/ned/tabsettings.php',
        array('id' => $courseid)
    );

...
$title = get_string('delete', 'format_ned');
...
if ($process) {
    ...
    $coursesschema = $DB->get_records_sql('
        SELECT id FROM {format_ned_config}
        WHERE '.$DB->sql_compare_text('variable', 8).' = "colourschema"
        AND value = '.$delete
    );
    if ($coursesschema) {
        foreach ($coursesschema as $courseschema) {
            $DB->set_field('format_ned_config', 'value', 1, array('id' => $courseschema->id));
        }
    }
